...
phil.train <- phil %>% filter(!is.na(Response)) %>% select(-Set)
phil.test<- phil %>% filter(is.na(Response)) %>% select(-Set)
```


```{r}
tune.grid <- expand.grid(trials  = seq(5, 10, 1),
                        model  = "rules",
                        winnow  = FALSE,
                        cost  = seq(.5, 2, .5))

tune.grid1 <- expand.grid(sigma = seq(0.5, 7, .05),
                          C = seq(.5, 1.5, .5),
                          Weight = 1)
...
boost <- train(form=Response~., 
              data=(phil.train %>% select(-SiteNum)),
...
f_df1 <- as.data.frame(cbind(y_true = phil.train$Response, Id = phil.train$SiteNum))


f_df2 <- phil.preds
f_df2$Id <- as.integer(f_df2$Id)
f_df2$Predicted <- as.integer(f_df2$Predicted)

f_df1 <- f_df1 %>% left_join(f_df2)



F1_Score(f_df1$y_true, f_df1$Predicted, positive = 1)
```
...
```{r}
tune.grid <- expand.grid(cost = seq(0, 5, .25),
                         weight = 2)
