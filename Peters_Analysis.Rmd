...
```{r}
tune.grid <- expand.grid(trials  = 20,
                        model  = "rules",
                        winnow  = FALSE,
                        cost  = seq(.1, 1, .1))
...
```{r}
down_train <- downSample(x = phil.train[, -ncol(phil.train)],
                         y = phil.train$Response)

tune.grid <- expand.grid(kmax = c(9,10,11),
                         distance = c(1,2),
                         kernel = "optimal")
...
boost1 <- train(form=Response~., 
              data=(down_train %>% select(-Class, -Set)),
              method = "xgbLinear",
              trControl=tr.grid,
              preProc = c("center", "scale", "pca"),
              metric = "F",
